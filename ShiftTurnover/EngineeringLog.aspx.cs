...
{
    public partial class EngineeringLog : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
                ...
                if (!Page.IsPostBack)
            {
                ...
                if (Request["update"] == "delete")
                {
                    ...
                    lblConfirm.Text = "The Live Log has been successfully deleted.";
                    lblConfirm.Visible = true;
                }
                    if (Request["update"] == "edit")
                    {

                        lblConfirm.Text = "The Live Log has been successfully edited.";
                        lblConfirm.Visible = true;
                    }
        ...
        protected void LoadPastEventsGrid()
        {
            ...
            try
            {
                ...
                DataSet ds = _dm.GetDataSet("SelectShiftEngineeringLog_1");
        ...
        private void DeleteAttachment(string _AttachmentID)
        {
            try
            {
                ...
                _dm.GetDataSet("EditEnggAttachment");
            }
            catch (Exception ex)
            {
        ...
        private void DeleteAttachmentFile(string AttachmentID)
        {
            if (AttachmentID != "" && AttachmentID != null)
            {
                try
                {
                    ...
                    DataSet ds2 = dm2.GetDataSet("selectstoredenggfilename");
        ...
        private void AddAttachment(string _AttachmentID, int LiveLogID)
        {
            if ((this.fileUpload.PostedFile != null) && (this.fileUpload.PostedFile.ContentLength > 0))
            {
                ...
                DataSet ds = dm.GetDataSet("EditEnggAttachment");
        ...
        private void ShowAttachment(string AttachmentID)
        {
            ...
            if (AttachmentID != "" && AttachmentID != null)
            {
                DataModule dm2 = new DataModule();
                dm2.AddParameter("@attachmentid", SqlDbType.Int, System.Convert.ToInt32(AttachmentID));
                DataSet ds2 = dm2.GetDataSet("selectenggattachmentdetails");
        ...
        private void LoadLiveLog(string _LiveLogID)
        {
            try
            {
                ...
                DataSet ds;
                DataModule _dm = new DataModule();
                _dm.AddParameter("@EngineeringLogID", SqlDbType.Int, Convert.ToInt32(_LiveLogID));
                ds  = _dm.GetDataSet("SelectEnggLog");  
                if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow row in ds.Tables[0].Rows)
                    {
                       txtEvent.Text= row["Description"].ToString();
                       lblLiveLogID1.Text = "Update Engineering Log ID: ";
                        int _attid = 0;
            ...
            {
                ErrorHandler.LogErrorToDB((int)Session["personroleid"], Convert.ToInt32(_LiveLogID), "MainPage.LoadLiveLog", "SelectEnggLog", ex);
                Response.Redirect("CustomErrorPage.aspx");
            }

        }
        private void EditLiveLog(string _LiveLogID)
        {
            try
            {
                DataModule _dm = new DataModule();
                _dm.AddParameter("@action", SqlDbType.VarChar, "update");
                ...
                _dm.GetDataSet("editEngglivelog");
            }
            catch (Exception ex)
            {
        ...
        protected void EditLiveLog(int _LiveLogID)
        {
...

            {
                ...
                if (_LiveLogID > 0)
                {
                    dataModule.AddParameter("@EngineeringLogID", SqlDbType.Int, _LiveLogID);
                }
                ds = dataModule.GetDataSet("UpdateEnggLog");
            ...
            Response.Redirect("EngineeringLog.aspx?update=edit");
        }
        protected void InsertLiveLog()
        {
...

            {
                ...
                ds = dataModule.GetDataSet("InsertEnggLog");
                int _LivelogID = 0;
                if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow row in ds.Tables[0].Rows)
                    {
                        _LivelogID = Convert.ToInt32(row["EngineeringLogID"]);
                    }
                }
                if (fileUpload.HasFile)
                {
            ...
            Response.Redirect("EngineeringLog.aspx?update=success");
        }
                    ...
                    case "Delete":
                        {
                            ...
                            Response.Redirect("EngineeringLog.aspx?update=delete");
