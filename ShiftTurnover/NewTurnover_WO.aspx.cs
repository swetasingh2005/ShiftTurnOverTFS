...
{
    public partial class NewTurnover_WO : System.Web.UI.Page
    {
        ...
        protected void LoadSmartGrid()
        {
            ...
            { 
                ErrorHandler.LogErrorToDB((int)Session["personroleid"], 0, "NewTurnover_WO.LoadSmartGrid", "AddEditReportInfo", ex);
        ...
        protected void LoadStatus()
        {
            try
            {
                ...
                if (ds != null)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow row in ds.Tables[0].Rows)
                        {
                            rdoWOComplete.SelectedValue = row["WOComplete"].ToString();
                            rdoWOCorrect.SelectedValue = row["WOCorrect"].ToString();
                        }
                    }
                }
            }
            ...
            {
                ErrorHandler.LogErrorToDB((int)Session["personroleid"], 0, "NewTurnover_WO.LoadStatus", "SelectReportDetails", ex);
                Response.Redirect("CustomErrorPage.aspx");
            }
        ...
        public void SaveStatus( )
        {
            try
            {
                ...
                _dm.AddParameter("@WOComplete", SqlDbType.VarChar, rdoWOComplete.SelectedValue);
                _dm.AddParameter("@WOCorrect", SqlDbType.VarChar, rdoWOCorrect.SelectedValue);
