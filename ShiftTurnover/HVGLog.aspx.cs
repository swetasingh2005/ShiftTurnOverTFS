...
{
    public partial class HVGLog : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
                ...
                if (!Page.IsPostBack)
            {
                if (Request["update"] == "success")
                {
                    ...
                    lblConfirm.Text = "The HVG Log has been successfully submitted.";
                    lblConfirm.Visible = true;
                }
                if (Request["update"] == "delete")
                {

                    lblConfirm.Text = "The HVG Log has been successfully deleted.";
                    lblConfirm.Visible = true;
                }
                    if (Request["update"] == "edit")
                    {

                        lblConfirm.Text = "The HVG Log has been successfully edited.";
                        lblConfirm.Visible = true;
                    }
        ...
        protected void LoadPastEventsGrid()
        {
            ...
            try
            {
                ...
                DataSet ds = _dm.GetDataSet("SelectShiftHVGLog");
        ...
        private void DeleteAttachment(string _AttachmentID)
        {
            try
            {
                ...
                _dm.GetDataSet("EditHVGAttachment");
            }
            catch (Exception ex)
            {
                ErrorHandler.LogErrorToDB((int)Session["personroleid"], Convert.ToInt32(_AttachmentID), "HVGLog.DeleteAttachment", "EditHVGAttachment", ex);
                Response.Redirect("CustomErrorPage.aspx");
            }
        ...
        private void DeleteAttachmentFile(string AttachmentID)
        {
            if (AttachmentID != "" && AttachmentID != null)
            {
                try
                {
                    ...
                    DataSet ds2 = dm2.GetDataSet("selectstoredHVGfilename");
                ...
                {
                    ErrorHandler.LogErrorToDB((int)Session["personroleid"], Convert.ToInt32(AttachmentID), "HVGLog.DeleteAttachmentFile", "selectstoredHVGfilename", ex);
                    Response.Redirect("CustomErrorPage.aspx");
                }
        ...
        private void AddAttachment(string _AttachmentID, int LiveLogID)
        {
            if ((this.fileUpload.PostedFile != null) && (this.fileUpload.PostedFile.ContentLength > 0))
            {
                ...
                DataSet ds = dm.GetDataSet("EditHVGAttachment");
        ...
        private void ShowAttachment(string AttachmentID)
        {
            ...
            if (AttachmentID != "" && AttachmentID != null)
            {
                DataModule dm2 = new DataModule();
                dm2.AddParameter("@attachmentid", SqlDbType.Int, System.Convert.ToInt32(AttachmentID));
                DataSet ds2 = dm2.GetDataSet("selectHVGattachmentdetails");
        ...
        private void LoadLiveLog(string _LiveLogID)
        {
            try
            {
                ...
                DataSet ds;
                DataModule _dm = new DataModule();
                _dm.AddParameter("@HVGLogID", SqlDbType.Int, Convert.ToInt32(_LiveLogID));
                ds  = _dm.GetDataSet("SelectHVGLog");  
                if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow row in ds.Tables[0].Rows)
                    {
                       txtEvent.Text= row["Description"].ToString();
                       lblLiveLogID1.Text = "Update HVG Log ID: ";
                        int _attid = 0;
            ...
            {
                ErrorHandler.LogErrorToDB((int)Session["personroleid"], Convert.ToInt32(_LiveLogID), "HVGLog.LoadLiveLog", "SelectEnggLog", ex);
                Response.Redirect("CustomErrorPage.aspx");
            }
        ...
        protected void EditLiveLog(int _LiveLogID)
        {
...

            {
                ...
                if (_LiveLogID > 0)
                {
                    dataModule.AddParameter("@HVGLogID", SqlDbType.Int, _LiveLogID);
                }
                ds = dataModule.GetDataSet("UpdateHVGLog");
            ...
            {
                ErrorHandler.LogErrorToDB((int)Session["personroleid"], 0, "HVGLog.btnSubmit_Click", "InsertLiveLog", ex);
                Response.Redirect("CustomErrorPage.aspx?msg=" + ex.Message);

            }

            Response.Redirect("HVGLog.aspx?update=edit");
        }
        protected void InsertLiveLog()
        {
...

            {
                ...
                ds = dataModule.GetDataSet("InsertHVGLog");
                int _LivelogID = 0;
                if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow row in ds.Tables[0].Rows)
                    {
                        _LivelogID = Convert.ToInt32(row["HVGLogID"]);
                    }
                }
                if (fileUpload.HasFile)
                {
            ...
            Response.Redirect("HVGLog.aspx?update=success");
        }
                    ...
                    case "Delete":
                        {
                            ...
                            Response.Redirect("HVGLog.aspx?update=delete");
