...
{
    public partial class NewTurnover_LOTO : System.Web.UI.Page
    {
        ...
        {
            string ds  = CreateServiceRequest();
                ...
                if (theDataSet != null)
            {
                if (theDataSet.Tables[0].Rows.Count > 0)
                {
                        ...
                        DataTable results = theDataSet.Tables[0].Select(" nih_lototracking.lotonum  LIKE '%-%' ").CopyToDataTable();
                        results.DefaultView.Sort = "nih_lototracking.lockboxnum ASC";
                        if (results != null)
                        {
        ...
        {
            return date.ToUniversalTime().ToString("MM/dd/yyyy");
        }
        ...
        protected void grLOTO_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                string getValue = Convert.ToString(e.Item.Cells[6].Text);
                if (getValue.Length > 0)
                {
                    DateTime dt = Convert.ToDateTime(getValue);
                    e.Item.Cells[5].Text = GetUtcFormattedDate(dt);
                }
            }
