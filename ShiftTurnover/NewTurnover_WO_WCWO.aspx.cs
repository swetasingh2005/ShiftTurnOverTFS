...
{
    public partial class NewTurnover_WO_WCWO : System.Web.UI.Page
    {
        ...
        protected void LoadSmartGrid()
        {
            ...
            {
                ErrorHandler.LogErrorToDB((int)Session["personid"], "NewTurnover_WO_WCWO.LoadSmartGrid", "SelectMaximoMaintWO", ex);
                Response.Redirect("CustomErrorPage.aspx");
        ...
        protected void LoadStatus()
        {
            try
            {
                ...
                if (ds != null)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow row in ds.Tables[0].Rows)
                        {
                            rdoAlarmsYN.SelectedValue = row["ChemWOYN"].ToString();
                            txtAlarmsComments.Text = row["ChemWOComments"].ToString();
                        }
            ...
            {
                ErrorHandler.LogErrorToDB((int)Session["personid"], "NewTurnover_WO_WCWO.LoadStatus", "SelectReportDetails", ex);
                Response.Redirect("CustomErrorPage.aspx");
            }
        ...
        public void SaveStatus( )
        {
            try
            {
                ...
                _dm.AddParameter("@ChemWOYN", SqlDbType.VarChar, rdoAlarmsYN.SelectedValue);
                _dm.AddParameter("@ChemWOComments", SqlDbType.VarChar, Comments);
                _dm.ExecuteCommand("AddEditReportInfo");
            }
            catch (Exception ex)
            {
                ErrorHandler.LogErrorToDB((int)Session["personid"], "NewTurnover_WO_WCWO.SaveStatus", "AddEditReportInfo", ex);
                Response.Redirect("CustomErrorPage.aspx");
